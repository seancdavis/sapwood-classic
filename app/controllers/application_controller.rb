class ApplicationController < ActionController::Base
  ...
  private
    ...
    def redirect_route
      ...
      site_pages_path(current_site)
    end

    def verify_xhr
      redirect_to site_pages_path(current_site) unless request.xhr?
    end
    ...
    def all_blocks_and_pages
      @all_blocks_and_pages ||= current_site.pages
    end
    ...
    def current_site
      @current_site ||= begin
        return nil unless params[:site_id]
        all_sites.select { |s| s.id == params[:site_id].to_i }.first
      end
    end
