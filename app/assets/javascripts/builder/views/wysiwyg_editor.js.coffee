class App.Views.WysiwygEditor extends Backbone.View
  ...
  initialize: (options) ->
    ...
    @editor = new wysihtml5.Editor 'wysihtml5-textarea',
      ...
      stylesheets: [ WYSIWYG_BASE_STYLES, WYSIWYG_CUSTOM_STYLES ]
    @interval = setInterval(@parseHTML, 5000)
    # @initEditor(options)
  ...
  initElements: (options) ->
    @page = $('.ajax-page.wysiwyg-editor')
    @wysiwygField = $(options.wysiwygField)
    @markdownField = $(options.markdownField)
    @textareaJQ = @page.find('textarea')
    @textarea = @textareaJQ[0]
    @storedHTML = @textarea.value

  parseHTML: =>
    if @textarea.value != @storedHTML
      url = $('#wysiwyg_editor').data('autosave')
      data = $('#wysiwyg_editor').serialize()
      @storedHTML = @textarea.value
      $.post url, data, (result) =>
        @markdownField.val(result.markdown)
        @wysiwygField.val(@textarea.value)
